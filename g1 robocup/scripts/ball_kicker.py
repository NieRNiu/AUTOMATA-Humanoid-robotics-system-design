...
class RobotState(Enum):
    INIT = 0                    # 初始化状态
    NAVIGATE_TO_BALL = 1        # 移动到球前方
    ADJUST_ORIENTATION = 2      # 调整朝向
    KICK_BALL = 3               # 踢球
    DONE = 4                    # 完成
...
class BallKicker:
    def __init__(self):
        # 初始化ROS节点
        rospy.init_node('ball_kicker', anonymous=True)
        
        # 成员变量 - 位置和姿态信息
        self.robot_position_ = None          # 机器人位置 (Point)
        self.robot_orientation_ = None       # 机器人姿态 (Quaternion)
        self.ball_position_ = None           # 足球位置 (真值)
        self.left_post_position_ = None      # 左门柱位置 (真值)
        self.right_post_position_ = None     # 右门柱位置 (真值)
        self.goal_midpoint_ = None           # 球门中点 (计算值)
        ...
        rospy.Subscriber('/ball_true_position', Point, self.ball_position_callback)
        rospy.Subscriber('/left_goalpost_true_position', Point, self.left_post_position_callback)
        rospy.Subscriber('/right_goalpost_true_position', Point, self.right_post_position_callback)
    ...
    def ball_position_callback(self, position):
        ...
        if self.ball_position_ is None:
            rospy.loginfo("[球位置] 获取到足球位置: (%.2f, %.2f, %.2f)", 
                         position.x, position.y, position.z)
        self.ball_position_ = position
    
    def left_post_position_callback(self, position):
        """处理左门柱位置信息"""
        if self.left_post_position_ is None:
            rospy.loginfo("[门柱位置] 获取到左门柱位置: (%.2f, %.2f, %.2f)", 
                         position.x, position.y, position.z)
        self.left_post_position_ = position
        self.update_goal_midpoint()
    
    def right_post_position_callback(self, position):
        """处理右门柱位置信息"""
        if self.right_post_position_ is None:
            rospy.loginfo("[门柱位置] 获取到右门柱位置: (%.2f, %.2f, %.2f)", 
                         position.x, position.y, position.z)
        self.right_post_position_ = position
        self.update_goal_midpoint()
    
    def update_goal_midpoint(self):
        """更新球门中点位置"""
        if self.left_post_position_ is not None and self.right_post_position_ is not None:
            # 计算两个门柱的中点
            new_goal_midpoint = np.array([
                (self.left_post_position_.x + self.right_post_position_.x) / 2,
                (self.left_post_position_.y + self.right_post_position_.y) / 2,
                (self.left_post_position_.z + self.right_post_position_.z) / 2
            ])
            
            # 仅当goal_midpoint_首次设置或有明显变化时才输出日志
            if self.goal_midpoint_ is None or not np.array_equal(np.round(self.goal_midpoint_, 2), np.round(new_goal_midpoint, 2)):
                self.goal_midpoint_ = new_goal_midpoint
                rospy.loginfo("[门柱位置] 计算门柱中点: (%.2f, %.2f, %.2f)", 
                             self.goal_midpoint_[0], self.goal_midpoint_[1], self.goal_midpoint_[2])
            else:
                # 无变化时直接更新值，不输出日志
                self.goal_midpoint_ = new_goal_midpoint
    ...
    def calculate_kick_position(self):
        ...
        踢球方向: 从踢球位置指向球门中点的方向
        """
        if self.ball_position_ is None or self.goal_midpoint_ is None:
            return None, None
        ...
        ball_pos = np.array([
            ...
            self.ball_position_.z
        ])
        
        # 计算从球指向球门中点的方向向量
        direction = self.goal_midpoint_ - ball_pos
    ...
    def move_to(self, target_pos):
        """控制机器人移动到目标位置"""
        if self.robot_position_ is None or target_pos is None:
            return False
        
        # 当前机器人位置
        current_pos = np.array([
            self.robot_position_.x,
            self.robot_position_.y,
            self.robot_position_.z
        ])
    ...
    def adjust_orientation(self, target_direction):
        """调整机器人朝向指定方向"""
        if self.robot_orientation_ is None or target_direction is None:
            return False
    ...
    def kick_ball_action(self):
        """执行踢球动作 (向前直走)"""
        if self.robot_position_ is None or self.goal_midpoint_ is None or self.ball_position_ is None:
            return False
        ...
        # 计算到目标(球门中点)的距离
        delta = self.goal_midpoint_ - current_pos
        distance = np.linalg.norm(delta[:2])  # 只考虑xy平面
        ...
        goal_center_y = self.goal_midpoint_[1]
        ...
        cmd = MoveCmd()
        cmd.vy = 1.0  # 全速前进
        cmd.vx = vx_compensation  # 基于球位置的横向补偿 左右移动补偿 球在左：0.3 球在右：-0.3
        cmd.vyaw = 0.0  # 不旋转
        self.cmd_vel_pub_.publish(cmd)
    ...
    def navigate(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.state_ == RobotState.INIT:
                if (self.robot_position_ is not None and 
                    self.robot_orientation_ is not None and 
                    self.ball_position_ is not None and 
                    self.goal_midpoint_ is not None):
                    ...
                    if self.kick_position_ is not None and self.kick_direction_ is not None:
                        rospy.loginfo("[导航] 初始化完成，计算踢球位置: (%.2f, %.2f, %.2f)", 
                                     self.kick_position_[0], self.kick_position_[1], self.kick_position_[2])
                        self.state_ = RobotState.NAVIGATE_TO_BALL
                    else:
                        rospy.logwarn("[导航] 计算踢球位置失败，请检查球和门柱位置")
                else:
                    missing = []
                    if self.robot_position_ is None:
                        missing.append("机器人位置")
                    if self.robot_orientation_ is None:
                        missing.append("机器人姿态")
                    if self.ball_position_ is None:
                        missing.append("球位置")
                    if self.goal_midpoint_ is None:
                        missing.append("门柱位置")
                    
                    rospy.logdebug("[导航] 等待信息: %s", ", ".join(missing))
