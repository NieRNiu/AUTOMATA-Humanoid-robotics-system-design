...
void IOROS::MoveCallback(const unitree_legged_msgs::MoveCmd &msg)
{
    ...
    // lx = std::max(std::min(lx, MAX_LINEAR_OUTPUT), -MAX_LINEAR_OUTPUT);
    // ly = std::max(std::min(ly, MAX_LINEAR_OUTPUT), -MAX_LINEAR_OUTPUT);
    rx = std::max(std::min(rx, MAX_ANGULAR_OUTPUT), -MAX_ANGULAR_OUTPUT);
